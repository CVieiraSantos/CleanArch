...
namespace CleanArchMvc.Application.Services
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task Add(CategoryDTO categoryDTO)
        {
            ...
            await _categoryRepository.CreateCategoryAsync(categoriesEntity);
        }

        public async Task Update(CategoryDTO categoryDTO)
        {
            var categoriesEntity = _mapper.Map<Category>(categoryDTO);
            await _categoryRepository.UpdateCategoryAsync(categoriesEntity);
        }

        public async Task Remove(int id)
        {
            var categoriesEntity = _categoryRepository.GetByIdAsync(id).Result;
            await _categoryRepository.RemoveCategoryAsync(categoriesEntity);
        }
    }
}
